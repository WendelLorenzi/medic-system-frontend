...
import { createVaccine } from '../../../firebase/firestore';

const RegisterVaccineForms = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const id = await createVaccine(formData);
    ...
    <div style={formularioStyle}>
      <form onSubmit={handleSubmit} style={{
        ...
        <div style={{
          ...
          <input type="text" id="nome" name="vaccine" style={{
            width: '23%',
            height: '46px',
            fontSize: '26px',
            border: '1px solid #ccc',
        ...
        <div>
          <button style={buttonStyle} onClick={handleSubmit}>Cadastrar</button>
        </div>
      </form>
    </div>
  );
